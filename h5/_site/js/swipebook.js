...
var _scrolling=0;//如果正在用FT Scroller垂直滚动，则_scrolling=1
...
Swipe.prototype = {
  ...
  setup: function() {
	...
	//alert(this.element.children[0].innerHTML);
	/*
  ...
  slide: function(index, duration, turnpage) {
	...
	} else if (this.deltaX && this.deltaX>=0) { //修复了手指放开后弹回的小Bug
		firstmove=index-1;
		lastmove=index+1;
	...
	style = el.style;
	style.display="block";
	style.webkitTransitionDuration = style.MozTransitionDuration = style.msTransitionDuration = style.OTransitionDuration = style.transitionDuration = duration + 'ms'; //每次新加载页面，如果当前页的Previous页面是单页，翻页的时候transition效果就会失效，这是由于layout-5这种模板被设为display:table, 只要去掉这个设置即可
	...
	if (lastmove>=0 && lastmove<n) {
		thepos=firstmovedirection*this.width+"px";
		//在IOS 6上发生touchend卡顿的原因已经查明，是由于同时修改三个Slide的translate3d，导致iPad瞬时的计算能力不足，将最后移动的那个Slide延时移动即可解决这一问题
		setTimeout(function(){
	...
	//Oliver: 如果采用遍历的原生代码，则会在手指离开时出现明显的滑动停顿，但此处修改令代码不可复用，且必须借助jQuerry，今后要想办法改进
	setTimeout(function(){
		if (index>=2) $(jq).slice(0,index-1).hide();
		if (index+2<n) $(jq).slice(index+2,n).hide();
	...
	// if the swipe doesn't prompt a page change, then it shouldn't prompt changepage and playslide
  ...
  goprev: function(delay) {
    ...
    if (this.index) this.slide(parseInt(this.index)-1, this.speed, -1);
  ...
  gonext: function(delay) {
    ...
    if (this.index < this.length - 1) this.slide(parseInt(this.index)+1, this.speed, 1); // if not last slide
  ...
  onTouchStart: function(e) {
	if (/\b(notouchstart|notouchall)\b/.test(e.target.className)) {//如果是Navigation这样的横滚动条，则不允许翻页
        return;
    }
  ...
  onTouchMove: function(e) {
	...
	if (/\b(notouchmove|notouchall)\b/.test(e.target.className)) {//如果是Navigation这样的横滚动条，则不允许翻页
        return;
    }
...
//console.log(this.deltaX);
	  ...
	  if (this.index-1>=0 && this.deltaX>0) this.slides[this.index-1].style.MozTransform = this.slides[this.index-1].style.webkitTransform = 'translate3d(' + (this.deltaX-this.width) + 'px,0,0)';
	  if (this.index+1<this.length && this.deltaX<0) this.slides[this.index+1].style.MozTransform = this.slides[this.index+1].style.webkitTransform = 'translate3d(' + (this.deltaX+this.width) + 'px,0,0)';
    } else if (e.touches[0].pageY - this.start.pageY<-100 && _scrollerOn==0 && !/\b(noscroll)\b/.test(e.target.className)) {
        openRail(); //引用了别的文件的JS
    } else if (e.touches[0].pageY - this.start.pageY>100) {
        closeRail();
    }
  ...
  onTouchEnd: function(e) {
	...
	if (/\b(notouchend|notouchall)\b/.test(e.target.className)) {//如果是Navigation这样的横滚动条，则不允许翻页
        return;
    }
	fingermoving==0;
    if (_sliding==1 || _scrolling==1) {
    ...
    var isValidSlide = 
          ...
          || Math.abs(this.deltaX) > this.width/4,        // or if slide amt is greater than 1/4 the width
    ...
    if (!this.isScrolling) {
		...
		var movementSpeed,flingDuration;
		...
		if (lastPosition && comparisonPosition && lastPosition.t != comparisonPosition.t) {
            ...
            if (!/\b(notouch|large)\b/.test(e.target.className) && _clicktoTurnPage=="on") {//如果是点击大图片或链接，则不翻页，也不调出下方的Rail
